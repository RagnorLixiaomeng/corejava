...
public class FocusPoint_02_JavaDefineSimpleGenericClass {
    ...
    public static void DefineSimpleGenericClass(){
        ...
        // 构造对象 - 带有构造器的普通类
        RagnorPair<String, Integer> demo = new RagnorPair<>("ragnor", 18);
        RagnorPair<String, String> demo2 = new RagnorPair<>();
        // 方法
...
class RagnorPair<T, U>{ // 类定义中的类型变量指定方法的返回类型以及域和局部变量的类型
    private T first;  // 使用类型变量, 类型变量使用大写形式且短
    private U second;

    public RagnorPair(){first = null; second = null;}
    public RagnorPair(T first, U second) {this.first = first; this.second = second;}
